...
public class Trigger_combo : MonoBehaviour
{/*
var whatWeHave , ArrayList;
...
function OnTriggerEnter (other,Collider){
    ...
    for(i=0;i<whatWeHave.Count; i++){
        if(whatWeHave[i].tag=="object1") allIn[0]=true;
        if(whatWeHave[i].tag=="object2") allIn[1]=true;
        if(whatWeHave[i].tag=="object3") allIn[2]=true;
    }

// see if the check array is all true's
var doIt = true;
    for(i=0; i<3; i++)if(! allIn[i]) doIt=false;
...
function OnTriggerExit(other:Collider)
{
    ...
    if (whatWeHave.IndexOf(other.gameObject) > -1) whatWeHave.Remove(other.gameObject);
}
*/
